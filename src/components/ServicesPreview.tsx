        ...
        <ScrollReveal delay={80}>
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 lg:gap-8 transform-gpu">
            {services.items.map((service) => (
              <div
                key={service.title}
                className="group relative flex flex-col overflow-hidden rounded-3xl bg-white/[0.06] border border-white/10 backdrop-blur-sm transition-all duration-500 hover:border-sage/40 hover:bg-white/[0.09] hover:shadow-[0_20px_50px_rgba(0,0,0,0.3)] transform-gpu"
              >
                {/* Card Image Area */}
